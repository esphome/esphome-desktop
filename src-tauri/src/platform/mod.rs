...
// The shared "how long may a probe take" bound, so callers outside `platform`
// (the ESPHome version probe in `update`) apply the same line the probes in
// here do rather than picking their own.
pub(crate) use health::PROBE_TIMEOUT;
...
/// `app_log_appender` in `logging.rs` already relies on. Does not create the
/// directory.
