...
# of our own on top.
#
# The app repairs a broken tree by deleting the packages we installed and
# reinstalling them (issue #330). That only works if it can tell our packages
# apart from Python's own, and nothing on disk distinguishes them: pip, esphome
# and every dependency sit side by side in site-packages, and pip's own metadata
# is exactly what is unreliable in the trees that need repairing. So capture the
# answer here, at the one moment it is knowable for certain, and ship it.
#
# Must run after the pip upgrade (so the manifest names the pip we actually
# ship, not the one the tarball came with) and before the first esphome install.
#
# The paths come from sysconfig rather than being spelled out because the layout
# is platform-specific (`lib/pythonX.Y/site-packages` + `bin` against
# `Lib\site-packages` + `Scripts`); recorded relative to the tree root so the
# reset needs no knowledge of either layout, and so the manifest survives the
# copy into the user's data dir on macOS and Linux.
write_base_manifest() {
    local platform="$1"
    local python_dir="$2"
    local python_bin="$3"
    ...
    "$python_dir/$python_bin" - "$python_dir" > "$python_dir/$BASE_MANIFEST" <<'PY'
import sys
import sysconfig
...
for key in ("purelib", "scripts"):
    ...
    try:
        ...
        raise SystemExit(f"{key} ({path}) is outside the Python tree ({root})")
    swept.append((rel, path))

out = [
    "# Everything that ships with the interpreter itself.",
    "# Generated by build-scripts/prepare_bundle.sh; do not edit by hand.",
    "#",
    "# `sweep` marks a directory the app's package reset cleans out; `keep`",
    "# marks an entry inside one that belongs to Python rather than to us, and",
    "# so must survive. Paths are relative to the Python tree root, POSIX",
    "# separators on every platform.",
]
out += [f"sweep {rel.as_posix()}" for rel, _ in swept]
for rel, path in swept:
    if not path.is_dir():
        continue
    for child in sorted(path.iterdir()):
        out.append(f"keep {(rel / child.name).as_posix()}")
